...
/* GET TEST PARAMETERS */
export const getTestParameters = async ({token, id}: { token: string, id: string}): Promise<IFormattedTestParameters> => {
...
export const changeTestParameters = async ({ token, updates }: { token: string, updates: IFormattedTestParameters }): Promise<IFormattedTestParameters> => {
  const teacher = await getTeacherFromToken(token)
